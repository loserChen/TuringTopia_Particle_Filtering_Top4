...
for i in range(16,25):
    df['t_diff_{}'.format(i)] = df.groupby('event_id')['tlabel'].diff(periods=i).fillna(method='bfill')
df.sort_values(['event_id','cumsum'],inplace=True)
for i in range(8,22,2):
    df['hitid_diff_{}'.format(i)] = df.groupby('event_id')['t1'].diff(periods=i).fillna(method='bfill')

slope,intercept = -0.38253218,7.73506202
df['point_line']=df.apply(lambda row : math.fabs(slope*row['x'] - row['t'] + intercept)/math.pow(slope**2+1,0.5),axis=1)

def group_fea(df,target):
    df['event_id_{}_min'.format(target)] = df.groupby('event_id')[target].transform('min')
    df['event_id_{}_max'.format(target)] = df.groupby('event_id')[target].transform('max')
    df['event_id_{}_median'.format(target)] = df.groupby('event_id')[target].transform('median')
    df['event_id_{}_mean'.format(target)] = df.groupby('event_id')[target].transform('mean')
    df['event_id_{}_std'.format(target)] = df.groupby('event_id')[target].transform('std')
    print("**************************{}**************************".format(target))
...
def diff_stat(target):
    df['{}_min_diff'.format(target)] = df[target] - df['event_id_{}_min'.format(target)]
    df['{}_max_diff'.format(target)] = df['event_id_{}_max'.format(target)] - df[target]
    df['{}_median_diff'.format(target)] = df['event_id_{}_median'.format(target)] - df[target]
    df['{}_mean_diff'.format(target)] = df['event_id_{}_mean'.format(target)] - df[target]
# # x,y,t,q "偏移"
diff_stat('x')
diff_stat('y')
diff_stat('t')
...
print("**************************{}**************************".format('diff finish!'))

df = df.sort_values(by=['event_id', 't_h_2_min_diff']).reset_index(drop=True)
#t_h_2特征diff，强特
for i in range(10, 18):
    df['t_h_2_diff_last_{}'.format(i)] = df.groupby(['event_id'])['t_h_2'].diff(periods=i).fillna(0)
...
# 时间变化特征, 强特
for i in range(4,16):
    df['t_diff_last_{}'.format(i)] = df.groupby('event_id')['t'].diff(periods=i).fillna(0)
...
df = df.sort_values(by=['event_id', 'x_minus_xcmc_2_min_diff']).reset_index(drop=True)
for i in range(4, 15):
    df['x_minus_xcmc_2_diff_last_{}'.format(i)] = df.groupby(['event_id'])['x_minus_xcmc_2'].diff(periods=i).fillna(0)

#sort x, y
df = df.sort_values(by=['event_id', 'x', 'y']).reset_index(drop=True)
for i in [1, 2, 3, 4, 5, 6, 7, 8]:
    df['x_y_diff_last_{}'.format(i)] = df.groupby('event_id')['t'].diff(periods=i).fillna(0)


# 位置的变化特征, 线上 +3 左右
#
for i in range(5, 18):
    df['x_diff_last_{}'.format(i)] = df.groupby(['event_id'])['x'].diff(periods=i).fillna(0)
for i in range(1,10):
    df['y_diff_last_{}'.format(i)] = df.groupby(['event_id'])['y'].diff(periods=i).fillna(0)


print("**************************{}**************************".format('position change finish!'))
...
for i in range(10, 15):
    df['dis2c_diff_last_{}'.format(i)] = df.groupby(['event_id'])['dis2c'].diff(periods=i).fillna(0)

#这个特征是比较有用的 event_id 特征

df['event_id_realhit_ratio'] = df['event_id_nhitreal'] / df['event_id_nhit']
df['event_id_fakehit_ratio'] = 1 - df['event_id_nhitreal'] / df['event_id_nhit']
...
print("**************************{}**************************".format('xjb finish!'))

df['slope'] = df['y']/df['x']
df['slope_mc'] = (df['y']-df['event_id_ycmc']) / (df['x']-df['event_id_xcmc'])
...
df['x_cmc'] = (df['x']-df['event_id_xcmc'])/(df['event_id_xcmc']+df['event_id_xcmc'].mean())

df['y_cmc'] = (df['y']-df['event_id_ycmc'])/(df['event_id_ycmc']+df['event_id_ycmc'].mean())

df['nhit_bias'] = (df['event_id_nhit']-df['event_id_nhitreal'])/(df['event_id_nhitreal']+df['event_id_nhitreal'].mean())
...
useless_features = [
    'z', 'x_y', 'event_id_t_min', 'event_id_t_max',
    'event_id_nhit', 'event_id_y_min', 'event_id_x_min', 'event_id_y_max','event_id_x_max',
    'event_id_q_std','event_id_q_max', 'event_id_y_median', 'event_id_x_median',
    'x_max_diff', 'y_max_diff','dis_mc','event_id_terror_min', 'one', 't1', 'tlabel',
    'l','lx','m','my','t_h','event_id_thetamc_cos','event_id_thetamc_sin','event_id_phimc_cos',
    'event_id_phimc_sin','event_id_x','event_id_x_minus_xcmc_2_std','event_id_x_minus_xcmc_2_max',
    'event_id_x_minus_xcmc_2_mean','event_id_t_h_2_std','event_id_dis2c_max'
]
...
print(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>  level 1 build lgb and train >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
lgb_index = 9
...
def run_lgb(df_train, df_test, use_features, params):
    ...
    for fold, (tr_ind, val_ind) in enumerate(folds.split(train, train[target], train['event_id'])):
        ...
        y_pred += model.predict(df_test[use_features], model.best_iteration) / folds.n_splits
        df_test['lgb_pred'] = model.predict(df_test[use_features], model.best_iteration)
...
def run_lgb_single(df_train, df_test, use_features, params):
    target = 'flag'
    start_time = time.time()
    x_train, x_val, y_train, y_val = train_test_split(df_train[use_features], df_train[target], test_size=0.2,
                                                      random_state=0)
    train_set = lgb.Dataset(x_train, y_train)
    val_set = lgb.Dataset(x_val, y_val)
...
def eval(row,margin_num):
    if (row['nhitreal'] / row['nhit']) > 0.47 and row['n'] <= (
            row['nhitreal'] + margin_num - int((row['nhitreal'] / row['nhit'] - 0.47) * 7)):
        return 1
...
def write_to_csv(df_test, df_event, margin_num=8):
    ...
    df_test[['hit_id', 'flag_pred', 'event_id']].to_csv('sub_nhit_{}.csv'.format(margin_num), index=False)

test['lgb_pred'] = y_pred
train['lgb_pred'] = oof_pred
train[['hit_id', 'flag_pred', 'event_id']].to_csv('train_lgb_{}_{}.csv'.format(score,str(lgb_index)), index=False)
test[['hit_id', 'flag_pred', 'event_id']].to_csv('test_lgb_{}_{}.csv'.format(score,str(lgb_index)), index=False)

#lgb产生预测结果
for i in [2,3,4]:
    write_to_csv(test[['hit_id', 'flag_pred', 'event_id']], event, margin_num=i)
    print(i)
...
#
# print(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>  level 1 build rf and train >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
#
